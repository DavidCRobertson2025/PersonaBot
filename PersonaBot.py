...
# ================================================================
# Global state variables for end user interviews
# ================================================================
...
def build_team_url(team_number):
    return f"https://robinn.co/AIhackTeam{team_number}"

def fetch_team_doc(team_number):
    url = build_team_url(team_number)
    try:
        response = requests.get(url)
        return response.text
    except Exception as e:
        print("Failed to fetch team doc:", e)
        return ""

def parse_team_doc(text):
    profile = ""
    product = ""

    if "END USER PROFILE" in text and "SELECTED PRODUCT OR SERVICE" in text:
        parts = text.split("SELECTED PRODUCT OR SERVICE")
        profile_part = parts[0].replace("END USER PROFILE", "").strip()
...
def find_audio_devices():
    ...
    for i in range(p.get_device_count()):
        ...
        print(i, name, "in:", max_in, "out:", max_out)
    ...
    print("\n[selected mic index]", AUDIO_INPUT_INDEX)
    print("[selected speaker index]", AUDIO_OUTPUT_INDEX)
...
def speak_text(ui, text: str, color=(0, 255, 0)):
    """
    Minimal, deterministic TTS playback using ALSA plughw.
    """
    ...
    # 2️⃣ Convert to 48kHz stereo WAV
    subprocess.run(
    ...
    # 3️⃣ Play through USB speaker (card 2) using plughw
    subprocess.run(
        ["paplay", wav_path]
    )
